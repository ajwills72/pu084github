...
```{r dist-mat, echo=FALSE, warning=FALSE}
## Load data
alldat  <- read_csv("ply84data.csv")
...
```{r echo=FALSE, message=FALSE, results='hide'}
munsell  <- read_csv("munsell-coords.csv")
munsell  <- munsell %>% mutate(chroma = chroma / 2) %>% select(-stim)
...
preds$os - preds.84$os
preds$bright - preds.84$bright
preds$chroma - preds.84$chroma
...
eda <- eda %>% add_column(exp = "ply044")
eda.ply62  <- eda.ply62 %>% add_column(exp = "ply062")
eda.ply84  <- bind_rows(eda, eda.ply62)
...
## Write out
write_csv(bigmf.ply84, "ply084fit.csv")
```

```{r ply084-model-block, echo=FALSE, message=FALSE, results='hide', cache=TRUE}
bigmf.ply84  <- read_csv("ply084fit.csv")
...
## Write out
write_csv(bigmfblk.ply84, "ply084fit-block.csv")
```

```{r ply84-block-report, echo=FALSE, message=FALSE}
## Load saved model fit
bigmfblk.ply84  <- read_csv("ply084fit-block.csv")
