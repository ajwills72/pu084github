...
```{r ply044-preproc, echo=FALSE, message=FALSE, results='hide'}
eda  <- read_csv("ply44data.csv")

## List of subject numbers by condition
ply044.N  <- eda %>% filter(!duplicated(subj)) %>% select(cond, subj)

```

```{r, echo=FALSE, message=FALSE}
# leaving `r  ply044.N %>% filter(cond == 1) %>% summarise(n())` participants in the high time pressure condition, and `r  ply044.N %>% filter(cond == 2) %>% summarise(n())` in the low time pressure condition.
```
...
## Extract mapping of stim code to physically presented stimulus file names, from raw data
preds  <- edag %>% filter( `Procedure[Block]` == "blockproc") %>%
    select(triadidmain, top, mid, bot)
colnames(preds)  <- c("triad", "left1", "mid2", "right3")
...
## Subsequent code previously used hand-coded predictions loaded from
## ply44code.csv. 
## cde  <- read_csv("ply44code.csv")
cde  <- preds 
...
## so.
bigdta  <- eda
ppts  <- as.numeric(ply044.N$subj)
bigmod <- array(0, dim=c(length(ppts),14))
bigmod <- t(mapply(ppfits, ppts, 0)) ## This line does the model fits
colnames(bigmod) <- c("subj","cond","blk","bright", "chroma","ud",
                      "os", "ident", "resp1", "resp2", 
                      "resp3", "model", "consist","margin")

## Convert output to more informative labels
bigmf <- as.data.frame(bigmod)
bigmf$model[bigmf$model > 3] <- 'Bias'
bigmf$model[bigmf$model == 1] <- 'UD'
bigmf$model[bigmf$model == 2] <- 'OS'
bigmf$model[bigmf$model == 3] <- 'ID'
bigmf$cond[bigmf$cond==1] <- '100 ms'
bigmf$cond[bigmf$cond==2] <- '2000 ms'

## Write out
write_csv(bigmf, "ply044fit.csv")
```

```{r ply44-model, echo=FALSE, message=FALSE}
bigmf  <- read_csv("ply044fit.csv")
mf.calc  <- table(bigmf$cond, bigmf$model)  # Cross tab

## Make table for purposes of display
mf.sum  <- as.data.frame(mf.calc)
colnames(mf.sum)  <- c("Time", "Model", "Freq") # Name columns
## Adding absence of ID model
mf.sum  <- mf.sum %>% add_row(Time = "100 ms", Model = "ID", Freq = 0)
mf.sum  <- mf.sum %>% add_row(Time = "2000 ms", Model = "ID", Freq = 0)
mf.sum  <- mf.sum %>% pivot_wider(names_from = Model, values_from = Freq)

mf.sum  <- mf.sum %>% select(Time, UD, OS, ID, Bias)

knitr::kable(
           mf.sum, row.names=FALSE, digits=2,
...
## Bayesian chi-square
mf.bf  <- contingencyTableBF(mf.calc[,2:3],
                             fixedMargin="rows",
                             sampleType="indepMulti")
mf.bf.val  <- round(extractBF(mf.bf)$bf, 2)
```

Due to technical issues, the data for two participants were not recorded. Table \@ref(tab:ply44-model) shows, as expected, that there was a strong preference for overall similarity classification at 2000 ms. However, shortening the presentation time to 100 ms appeared to have little effect. One participant was best fit by a response-bias model (i.e. always pressing the same key), one by the assumption they were responding on the basis of a single dimension. All the other participants remained best fit by an overall similarity account. Bayesian analysis, conducted with the `BayesFactor` R package [@R-BayesFactor], indicated that the ratio of unidimensional to overall similarity classifiers was unaffected by our time pressure manipulation, _BF_ = `r mf.bf.val`. This result is not diagnostic between Combination Theory and Differentiation Theory. Differentiation Theory can attribute the lack of increase in overall-similarity classification to a ceiling effect, and Combination Theory can attribute it to the time pressure manipulation being insufficiently potent to produce a detectable effect.
...
## so.
bigdta  <- eda
bigdta <- bigdta[bigdta$subj != 44020,] # Remove response bias participant
ppts.blk <- levels(as.factor(bigdta$subj))
bigmodblk <- t(mapply(ppfits,rep(ppts,each=12),1:12)) # Fit to each block
colnames(bigmodblk) <-colnames(bigmod)

# Convert to more informative labels
bigmfblk <- as.data.frame(bigmodblk)
bigmfblk$model[bigmfblk$model == 1] <- 'UD'
bigmfblk$model[bigmfblk$model == 2] <- 'OS'
bigmfblk$model[bigmfblk$model == 3] <- 'ID'
bigmfblk$cond[bigmfblk$cond==1] <- '100 ms'
bigmfblk$cond[bigmfblk$cond==2] <- '2000 ms'

## Write out
write_csv(bigmfblk, "ply044fit-block.csv")
```

```{r ply44-block-report, echo=FALSE, message=FALSE}
## Load saved model fit
bigmfblk  <- read_csv("ply044fit-block.csv")

## Remove response bias fit columns - these analyses were not conducted.
bigmfblk  <- bigmfblk %>% select(-resp1, -resp2, -resp3)

## Figures for reporting
N.ties  <- bigmfblk %>% filter(margin == 0) %>% summarise(N= n())
N.ppts  <- length(unique(bigmfblk$subj))
N.blks  <- length(unique(bigmfblk$blk))

## Remove tied blocks
bigmfblk  <- bigmfblk %>% filter(margin > 0)

## Aggregate for analysis

## Count model wins
blk.sum  <- as.data.frame(table(bigmfblk$subj, bigmfblk$model))
colnames(blk.sum)  <- c("subj", "Model", "Freq")
blk.sum  <- blk.sum %>% pivot_wider(names_from=Model, values_from=Freq)

## Bind condition data back into frame
conds  <- bigmfblk %>% select(subj, cond) %>% distinct
conds$subj  <- as.factor(conds$subj)
blk.sum  <- right_join(conds, blk.sum)

## Calculate proportions (note: some missing blocks due to ties)
blk.sum  <- blk.sum %>% mutate(total = ID+OS+UD)
blk.sum  <- blk.sum %>% mutate(ID.p = ID/total, OS.p = OS/total, UD.p = UD/total)

blk.report  <- blk.sum %>% group_by(cond) %>%
    summarise(UD = mean(UD.p),
              OS = mean(OS.p),
              ID = mean(ID.p)) 

## Produce table for report
colnames(blk.report)  <- c("Time", "UD", "OS", "ID")

knitr::kable(
           blk.report, row.names=FALSE, digits=3,
...
eda.ply62  <- read_csv("ply62data.csv")
...
## Power calclaultion on basis of ply044 effect size
ud.cohen  <- cohen.d(blk.sum$UD.p ~ blk.sum$cond)

pwr.ply062  <- pwr.t2n.test(d=ud.cohen$estimate, alternative="greater", n1 = ply062.n1,
                            n2 = ply062.n2)
...
Fifty psychology students from Plymouth University participated for partial course credit; this sample size has sufficient (`r round(pwr.ply062$power*100, 0)`%) power to detect the medium-to-large effect observed in the by-blocks analysis of the previous experiment (d = `r round(ud.cohen$estimate, 2)`). The stimuli, apparatus, and procedure, were identical to the previous experiment.
...
## Write out
write_csv(bigmf.ply62, "ply062fit.csv")
```

```{r ply062-model-block, echo=FALSE, message=FALSE, results='hide', cache=TRUE}
bigmf.ply62  <- read_csv("ply062fit.csv")
...
## Write out
write_csv(bigmfblk.ply62, "ply062fit-block.csv")
```

```{r ply62-block-report, echo=FALSE, message=FALSE}
## Load saved model fit
bigmfblk.ply62  <- read_csv("ply062fit-block.csv")
...
mean_theory  <- as.numeric(blk.report[1, 'UD'] - blk.report[2, 'UD'])
